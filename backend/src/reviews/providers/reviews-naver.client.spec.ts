...
import { ReviewsNaverClient } from "./reviews-naver.client";
...
describe("ReviewsNaverClient", () => {
  ...
  it("뉴스 검색 응답을 source candidate로 정규화한다", async () => {
    ...
    const client = new ReviewsNaverClient();
  ...
  it("originallink가 없으면 link를 canonicalUrl로 사용한다", async () => {
    global.fetch = jest.fn().mockResolvedValue(
      createFetchResponse({
        jsonData: {
          items: [
    ...
    expect(result[0]).toMatchObject({
      canonicalUrl: "https://n.news.naver.com/mnews/article/001/0010000001",
      originalUrl: "https://n.news.naver.com/mnews/article/001/0010000001",
      publishedAt: null,
    });
  });

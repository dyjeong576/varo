...
- 사용자 질문 문장을 그대로 줄이지 말고 뉴스 검색 의도로 변환함
- "누구", "왜", "어떻게", "알려줘", "궁금해" 같은 질문어·요청어는 제거함
- 고유명사, 사건·행위, 정책·제도, 수치·발표, 쟁점·결과를 우선 보존함
- 직책·소속·상태 표현은 뉴스 제목에서 흔한 형태로 정규화하되 특정 표현을 억지로 추가하지 않음
- 설명형 질문은 인물/용어 소개보다 사용자가 함께 언급한 사건·쟁점·배경을 우선 검색함
- 하나의 쿼리에 서로 다른 의도를 섞지 말고 query마다 하나의 검색 목적만 담음
- 4개 쿼리는 정확 사건, 최신 쟁점, 공식/원출처, 반박/해명/후속 보도 관점으로 다변화함

...
export class AnswerQueryArtifactDto {
...
export class AnswerCandidateDto {
  ...
  @ApiProperty({
    ...
    example: "핵심 check과 직접 관련된 제목 또는 공식 출처 신호가 확인됩니다.",
    nullable: true,
  })
  ...
  @ApiProperty({
    description: "이 source가 check에 대해 보이는 방향성",
    example: "support",
...
export class AnswerEvidenceSnippetDto {
...
export class AnswerInterpretationHandoffDto {
  @ApiProperty({ description: "정제된 핵심 check", example: "테슬라의 한국 시장 철수" })
  coreCheck!: string;
...
export class AnswerSourceBreakdownDto {
...
export class AnswerAnalysisResultDto {
  @ApiProperty({
  ...
  @ApiProperty({
    description: "LLM structured output으로 생성한 결과 화면용 uncertainty 요약",
    example:
  ...
  @ApiProperty({
    ...
    type: AnswerSourceBreakdownDto,
  })
  sourceBreakdown!: AnswerSourceBreakdownDto;
}

export class AnswerQueryProcessingPreviewResponseDto {
  @ApiProperty({ description: "answer job 식별자", example: "answer-1" })
  answerId!: string;

  @ApiProperty({
    description: "client에서 생성한 요청 식별자. local pending draft와 server answer job 병합에 사용합니다.",
    example: "pending:answer-1",
    nullable: true,
  })
  clientRequestId!: string | null;

  @ApiProperty({ description: "check 식별자", example: "check-1" })
  checkId!: string;

  @ApiProperty({
    description: "사용자가 입력한 check 원문",
    example: "나 어제 뉴스에서 봤는데 테슬라가 한국에서 완전 철수한대",
  })
  rawCheck!: string;

  @ApiProperty({
    description: "answer job 생성 시각",
    example: "2026-04-01T02:00:00.000Z",
  })
  createdAt!: string;

  @ApiProperty({ description: "answer 상태", example: "partial" })
  status!: string;
  ...
  @ApiProperty({ description: "후보 source 목록", type: [AnswerCandidateDto] })
  sources!: AnswerCandidateDto[];

  @ApiProperty({ description: "생성된 evidence snippet 목록", type: [AnswerEvidenceSnippetDto] })
  evidenceSnippets!: AnswerEvidenceSnippetDto[];
  ...
  @ApiProperty({
    ...
    type: AnswerInterpretationHandoffDto,
  })
  handoff!: AnswerInterpretationHandoffDto;

  @ApiProperty({
    description:
      "최종 truth 판정이 아닌, 현재 수집된 출처를 기준으로 계산한 임시 결과 화면용 분석 데이터. MVP 지원 범위 밖이면 null입니다.",
    type: AnswerAnalysisResultDto,
    nullable: true,
  })
  result!: AnswerAnalysisResultDto | null;
}

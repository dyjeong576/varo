...
3. 분석 과정에서 `search_route=news`이면 Naver provider가 먼저 호출되고, Naver 후보가 부족할 때만 Tavily fallback provider가 호출된다.
...
5. 현재 프론트는 `handoff_ready` 상태의 answer preview detail과 `rule_based_preview` 파생 결과를 우선 소비한다.

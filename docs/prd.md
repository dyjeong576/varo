...
이번 MVP는 한국어 뉴스 소비자를 주요 대상으로 하되, 검토 범위는 국가와 무관한 뉴스성 claim 전체로 둔다. 한국 뉴스성 claim은 네이버 뉴스 검색 API를 기본 검색 경로로 사용하고 해외/글로벌 뉴스성 claim은 Tavily를 사용한다. 글로벌 검색 route에서는 사용자-facing claim/query는 원문을 유지하고, 실제 Tavily 검색 입력만 영어 claim/query로 변환한다. 핵심 경험은 결과 페이지이며, 결론보다 근거를 먼저 이해할 수 있게 설계한다.
...
- 한국 뉴스성 claim
- 해외/글로벌 뉴스성 claim
- 공식 발표, 인터뷰 발언, 온라인 게시물처럼 사실성 검토가 가능한 claim
...
- 한국 뉴스성 claim은 네이버 뉴스 검색 API를 기본 검색 provider로 사용한다.
- 해외/글로벌 뉴스성 claim은 Tavily Search/Extract를 사용한다.

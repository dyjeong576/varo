...
- 출처 간 일치, 충돌, 불확실성 표시
- 결과 페이지 중심 UX
- 비로그인 사용자의 홈, answer 생성/조회, 히스토리, 인기 주제, 헤드라인 이용
- 로그인 전용 커뮤니티, 알림, 설정, 내 정보 경험
...
- 완전한 다국어 지원
- 뉴스성 또는 사실성 검토 대상이 아닌 일반 의견/상담/창작 요청에 대한 fact-check verdict 생성
- 지원 범위 밖 뉴스 check
...
- 비로그인 사용자는 회원가입 전에도 핵심 검토 흐름을 체험하고 싶다.
...
- 입력 단위는 `check`이다.
- 비로그인 사용자도 check 입력과 answer 생성 흐름을 이용할 수 있다.
- 비로그인 answer 생성은 게스트 쿠키 기준 일일 20회로 제한한다.
...
- 저장된 헤드라인 기사는 단일 테이블에 보관하되 `category=politics|economy` 필드로 정치/경제 범위를 명시적으로 구분한다.
...
- 분석 API는 저장된 RSS 제목만 기반으로 1차 사건 묶음을 만들고, OpenAI는 해당 cluster 후보의 사건명, 짧은 요약, 매체별 표현 차이만 정리한다.
- 헤드라인 분석 사건 묶음은 보도한 매체 수가 많은 사건 순으로 표시하고, 분석 대상 기사에서 누락된 기사가 없도록 한다.
...
- 사용자는 로그인하지 않아도 홈에서 check을 입력하고 answer 결과를 조회할 수 있어야 한다.
- 비로그인 사용자의 answer 기록은 같은 브라우저의 게스트 쿠키 기준으로 유지해야 한다.
...
- 시스템은 `answerMode=fact_check|direct_answer|context_answer_with_news`를 반환해야 한다.
- `answerMode=direct_answer`이면 시스템은 뉴스 검색과 verdict 생성을 수행하지 않고 OpenAI 직접 답변으로 처리해야 한다.
- `context_answer_with_news`는 한국 정치·경제 설명형 질문에만 사용하며, OpenAI 맥락 답변과 Naver 관련 뉴스를 함께 표시하되 verdict를 생성하지 않아야 한다.
...
### 11.7 Guest and Account Access

- 홈, answer 생성/조회, 히스토리, 인기 주제, 헤드라인은 비로그인 사용자도 접근할 수 있어야 한다.
- 커뮤니티, 알림, 설정, 내 정보, 온보딩 프로필은 로그인 전용으로 유지해야 한다.
- 게스트 answer 결과는 로그인 후 계정으로 자동 병합하지 않는다.
- 프로필 미완성 로그인 사용자도 홈, answer, 인기 주제, 헤드라인은 이용할 수 있어야 한다.

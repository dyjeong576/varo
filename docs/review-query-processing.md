...
- LLM provider: OpenAI
- 구조 원칙: **한국 정치·경제 뉴스성 claim**은 Naver News Search를 먼저 사용하고, Naver 후보가 충분하지 않을 때만 Tavily Search 보조검색을 사용한다. 한국 정치·경제 범위를 벗어나는 뉴스 claim은 `unsupported`로 처리한다.
...
- OpenAI가 생성하는 아티팩트는 `core_claim`, `normalized_claim`, `claim_type`, 목적별 `search_plan`, `search_route`이다.
- 생성 규칙은 아래를 따른다.
  - 원문 언어를 유지한다.
  - 구어체를 제거한다.
...
- `search_plan.queries[]`의 purpose는 아래 4개로 고정한다. `news` route는 정확히 4개를 생성하고, `unsupported`는 빈 배열을 반환한다.
  ...
  - `contradiction_or_update`: 취소·번복·정정·부인 등 claim을 반박하거나 업데이트하는 출처를 찾는다.
...
- `search_route`는 신규 생성 기준 아래 2개 중 하나로 고정한다.
  - `news`: **한국 정치·경제 뉴스성 claim**이며 Naver News Search를 기본 검색으로 사용한다.
  - `unsupported`: **한국 정치·경제** 뉴스성 또는 사실성 검토 대상이 아니거나 provider로 근거 수집이 불가능한 claim이다.
- 지원 범위 판정과 provider routing은 `search_route`가 authoritative field로 담당하고, 사용자-facing 설명은 `search_route_reason`에 남긴다.
- 한국 정치·경제 범위를 벗어나는 뉴스 claim은 `unsupported/out_of_scope`로 기록한다.
...
- OpenAI는 `core_claim`과 각 candidate의 `title`, `snippet`, `publishedAt`, `queryPurposes`, `publisherName`, `sourceType`을 입력으로 받아 해당 source의 relevance와 evidence signal을 단일 호출에서 판정한다. `search_route`, `source_provider`, `retrieval_bucket`은 LLM 입력에 포함하지 않는다.
...
sequenceDiagram
    ...
    ReviewAPI->>QueryPlanner: raw claim 기반 scope 판정 + normalized claim + search plan + search_route 단일 호출
    QueryPlanner-->>ReviewAPI: structured JSON 반환
    ReviewAPI->>DB: core_claim / search_plan / search_route 저장
    ...
    alt search_route=unsupported
        ...
        alt Naver 후보가 15건 미만
            ReviewAPI->>Tavily: 한국 뉴스 도메인 기반 fallback search
            Tavily-->>ReviewAPI: source 후보 반환
        end
        ReviewAPI->>ReviewAPI: 결과 병합 / canonical URL 정규화 / 중복 제거 / provider metadata 부여
        ReviewAPI->>RelevanceSignalClassifier: core claim + title/snippet/publishedAt/queryPurposes/publisherName/sourceType 기반 relevance / signal 분류 요청 (상한 8개)
...
sequenceDiagram
    ...
    alt appEnv != dev
        ...
        Providers-->>QueryPreviewService: core_claim + search_plan + search_route
        alt search_route=unsupported
...
| Claim Understanding, Scope Gate, and Search Planning | 정규화 claim | `core_claim`, `normalized_claim`, `claim_type`, `search_plan` (4개 purpose별 쿼리 포함), `search_route` |
...
| Relevance and Evidence Signal Classification | `core_claim`, `title`, `snippet`, `publishedAt`, `queryPurposes`, `publisherName`, `sourceType` | source별 `relevance_tier`, `relevance_reason`, `origin_query_ids[]`, origin query purpose, source-level evidence signal |

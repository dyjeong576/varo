...
대상은 인증, 사용자, answer, 커뮤니티, 인기, 알림, 히스토리, 공통 로그를 포함한 서비스 전반의 데이터 구조다.

## 2. 데이터 모델 원칙
- 서비스 전반은 하나의 사용자 계정 체계를 공유한다.
- answer 도메인은 `check`, `evidence`, `interpretation`, `uncertainty`를 분리 저장한다.
- source와 evidence snippet까지 추적 가능해야 한다.
- 커뮤니티와 알림은 사용자 행위와 answer 결과를 연결하는 보조 도메인이다.
...
| 컬럼 | 타입 | 설명 |
| --- | --- | --- |
| `id` | uuid / text id | answer 식별자 |
| `user_id` | fk | 요청 사용자 |
| `check_id` | fk | `checks.id` 참조 |
...
| `query_refinement` | jsonb nullable | check understanding, search plan, provider route audit |
...
| `answer_job_id` | fk | `answer_jobs.id` 참조 |
...
`EvidenceSnippet.stance`는 API 호환과 빠른 UI 표시를 위한 값이다. signal classification 이후에는 `support`, `conflict`, `context`, `unknown` 값을 우선 사용할 수 있고, 기존 answer에는 `neutral` 등 과거 값이 남을 수 있다. 현재 preview 생성 경로는 `evidence_snippets` row를 생성하지 않을 수 있으므로, 상세 판단 사유와 source stance는 `handoff_payload.evidenceSignals[]`에서 우선 추적한다.

### 6.8 `answer_results`

현재 query-processing preview 경로는 `answer_results` row를 저장하지 않고, `answer_jobs.query_refinement`, `answer_jobs.handoff_payload`, `sources`를 기반으로 `rule_based_preview` 결과를 생성 응답과 조회 응답에서 파생한다. 아래 테이블은 final interpretation 저장 단계 확장용 모델이다.
| 컬럼 | 타입 | 설명 |
| --- | --- | --- |
| `id` | uuid / text id | result 식별자 |
| `answer_job_id` | fk unique | `answer_jobs.id` 참조 |
...
| `answer_job_id` | fk nullable | 관련 answer |
...
| `title` | text | 제목 |
| `body` | text | 본문 |
| `linked_answer_job_id` | fk nullable | 관련 answer |
| `created_at` | timestamptz | 생성 시각 |
| `updated_at` | timestamptz | 갱신 시각 |
...
| `answer_completed` | boolean | answer 완료 알림 수신 여부 |
...
| `check_id` | fk nullable | 대표 check |
| `topic_text` | text | 인기 주제 텍스트 |
| `ranking_score` | numeric | 집계 점수 |
| `answer_count` | integer | 관련 answer 수 |
...
- browser localStorage `varo.answer-tasks`

## 7. traceability 기준
- 어떤 answer가 어떤 check에서 시작했는지 연결되어야 한다.
- 어떤 result가 어떤 source와 evidence signal에 근거했는지 추적 가능해야 한다. snippet row가 있는 경우에는 snippet까지 연결한다.
- 어떤 알림이 어떤 answer 또는 community 이벤트에서 생성되었는지 추적 가능해야 한다.
- history는 사용자와 answer 연결을 보존해야 한다.

## 8. 데이터 모델 요약
- `users`는 서비스 전체의 계정 기준점이다.
- `answer_jobs`는 answer 도메인의 중심 엔티티이며, 현재 preview 결과 파생에 필요한 `query_refinement`와 `handoff_payload`를 보관한다.
- `community_posts`는 answer 결과와 느슨하게 연결될 수 있다.
- `notifications`는 answer와 community 도메인을 사용자 액션으로 연결한다.
- 현재 프론트는 `answer_task_records`만 보조 상태 저장으로 사용한다.
